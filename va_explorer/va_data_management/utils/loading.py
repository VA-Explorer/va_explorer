...
from va_explorer.va_data_management.utils.coding import detect_va_format
...
def load_records_from_dataframe(record_df, random_locations=False, debug=True):
    ...
    # first, detect VA format. If unable to find a matching form, throw error
    va_format = detect_va_format(record_df)
    tf = time.time(); print(f"time for format detection: {tf - ti} secs"); ti = tf

    if 'error' in str(va_format.lower()):
        raise TypeError("ERROR: Data does not match any supported formats. Please ensure it is based on one of the WHO VA standards.")
    else:
        print(f"Detected VA format: {va_format}")
        ...
        # add detected format to VA data
        record_df['version'] = va_format
            ...
            if parent_node:
                ...
                if row_location:
                    ...
                    if row_location.get_parent().name != parent_node.name:
                        print(f"WARNING: Updating {row_location.name}'s parent to {parent_node.name}")
