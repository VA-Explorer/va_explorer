...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        vadata = VerbalAutopsy.objects.filter(causes__isnull=False).exclude(
            id__in=dhisdata
        )  # [:30]
        ...
        if len(va_not_in_dhis) > 0:
            ...
            for i in range(len(vadata["sex"])):
                vadata["dob"][i] = (
                    dateutil.parser.parse(vadata["dob"][i]).date().strftime("%Y-%m-%d")
                )
                vadata["dod"][i] = (
                    dateutil.parser.parse(vadata["dod"][i]).date().strftime("%Y-%m-%d")
                )
            ...
            self.clearFolder("DHIS/blobs/")
            if postLog["response"]["status"] == "SUCCESS" and postLog["response"][
                "imported"
            ] == len(va_not_in_dhis):
            ...
            self.stdout.write(
                f" Uploaded {numPushed} out of {numTotal} verbal autopsies "
            )
    ...
    def syncDHISStatus(self):
        ...
        dhisdata = dhisStatus.objects.filter(
