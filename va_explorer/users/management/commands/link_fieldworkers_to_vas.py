...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--emails",
            help="List of field worker emails to check for during linking. \
            Provide as one, comma-separated string. Ex. abc@email.com,def@email.com",
            type=argparse.FileType("r"),
        )
        parser.add_argument("--match_threshold", type=float, nargs="?", default=0.80)
    ...
    def handle(self, *args, **options):
        ...
        match_threshold = options.get("match_threshold", 0.80)
        debug = options.get("debug", False)
        link_fieldworkers_to_vas(
            emails=emails, match_threshold=match_threshold, debug=debug
        )
