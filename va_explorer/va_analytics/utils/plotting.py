...
def cod_group_plot(
    ...
    color_dict = {
        color_key: LOOKUP["color_list"][i] for i, color_key in enumerate(color_keys)
    }
    ...
    if va_df.size > 0:
        ...
        for i, cod_group in enumerate(cod_groups):
            ...
            if cod_data.size > 0:
                ...
                cod_pivot.index = [
                    LOOKUP["display_names"].get(x, x) for x in cod_pivot.index
                ]
                ...
                for j, demo_group in enumerate(demo_groups):
                    ...
                    counts = cod_pivot.loc[
                        cod_pivot[demo_group] > 0, ["cod", demo_group]
                    ]
                    counts = counts[
                        counts["cod"] != "All"
                    ]  # .sort_values(by=demo_group, ascending=True))

                    counts[f"{demo_group}_pct"] = np.round(
                        100 * counts[demo_group] / denominators, 2
                    )
                    counts["text"] = (
                        f"<i>{demo_group}</i><br>"
                    ...
                    if chosen_cod != "all":
                        ...
                        if chosen_cod in counts.index:
                            ...
                            widths[chosen_idx] = 3
                            counts["cod"][chosen_idx] = (
                                "<b>" + counts["cod"][chosen_idx] + "</b>"
                            )
                    ...
                    for k, trace_type in enumerate([demo_group, f"{demo_group}_pct"]):
                        ...
                        group_legend_name = LOOKUP["display_names"].get(
        ...
        buttons = create_percent_count_buttons(
            num_groups=int(len(figure.data) / 2), y=1, traces=figure.data
        )
...
def cause_of_death_plot(va_df, factor, N=10, chosen_cod="all", title=None, height=None):
    ...
    if va_df.size > 0:
        ...
        widths = np.repeat(1, len(counts["cod"]))
        if chosen_cod != "all":
            chosen_cod = LOOKUP["display_names"].get(
                chosen_cod, chosen_cod.capitalize()
            )
            if chosen_cod in counts.index:
                chosen_idx = counts.index.get_loc(chosen_cod)
                lines[chosen_idx] = "#e0b816"
        ...
        for i, group in enumerate(groups):
            if factor in ["all", "overall"]:
                ...
                counts[f"{group}_pct"] = counts.apply(
                    lambda row: np.round(100 * row[group] / row[groups].sum(), 1),
                    axis=1,
                )
            counts["text"] = (
                f"<i>{group}</i><br>"
        ...
        figure.update_layout(
            ...
            xaxis_title="Count",
            height=height,
            updatemenus=[create_percent_count_buttons(num_groups=len(groups))],
        )
    else:
    ...
    if va_df.size > 0:
        ...
        for term, ids in search_term_ids.items():
            ...
            for i, demo_group in enumerate(trend_counts.columns.tolist()):
                demo_group_name = LOOKUP["display_names"].get(
                    demo_group.lower(), demo_group.capitalize()
                )
                show_legend = (demo_group not in ["all", "overall"]) and (
                    demo_group_name not in legend_names
                )
                trace_data = plot_fn(
