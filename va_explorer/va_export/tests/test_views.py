...
class TestAPIView:
    ...
    def test_time_filter(self, rf:RequestFactory):
        ...
        request = rf.get(f"/va_export/verbalautopsy/?format=csv&start_date={start_date}&end_date={end_date}")
        request.user = User.objects.get(name='admin')
        response = va_api_view(request)
        ...
        db_ct = VerbalAutopsy.objects.filter(Id10023__gte=start_date, Id10023__lte=end_date).count()
        assert download_ct == db_ct
        # confirm all VAs are within chosen time period 
        assert ((df['Id10023'] >= start_date) & (df['Id10023'] <= end_date)).sum() == db_ct
    ...
    def test_combined_filter_csv(self, rf:RequestFactory):
        ...
        loc_a = Location.objects.get(name="Facility1")
        cod_name = "cod_a"
        comb_csv_query = f"format=csv&start_date={start_date}&locations={loc_a.pk}&causes={cod_name}"
        ...
        db_ct = VerbalAutopsy.objects.filter(Id10023__gte=start_date, causes__cause=cod_name, location__id=loc_a.pk).count()
        df = pd.read_csv(BytesIO(response.content))
        download_ct = df.shape[0]
        assert download_ct == db_ct
        # confirm all VAs are within chosen time period 
    ...
    def test_combined_filter_json(self, rf:RequestFactory):
        ...
        loc_a = Location.objects.get(name="Facility1")
        cod_name = "cod_b"
        comb_csv_query = f"format=json&end_date={end_date}&locations={loc_a.pk}&causes={cod_name}"
        request = rf.get(f"/va_export/verbalautopsy/?{comb_csv_query}")
        request.user = User.objects.get(name='admin')
        response = va_api_view(request)
        ...
        assert response.status_code == 200
        # confirm correct number of VAs downloaded
        db_ct = VerbalAutopsy.objects.filter(Id10023__lte=end_date, causes__cause=cod_name, location__id=loc_a.pk).count()
