...
# loc_query is a comma-separated list of either location ids or location names.
# If location names, perform name-based matching against all known locations in db.


# returns a list of location IDs matching location query.
def get_loc_ids_for_filter(loc_query):
    locs = loc_query.split(",")
    # first, assume locations are given as ids. If not, treat as names and
    ...
    try:
        ...
        # check query against all locations in db. If a match, get all
        # location descendants if not already facility
