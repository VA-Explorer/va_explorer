    ...
    def get_context_data(self, **kwargs):
        ...
        if user_vas.count() > 0:
            ...
            vas_to_address = user_vas.only('id', 'location_id', 'Id10007', 'Id10010', 'Id10017',
                'Id10018', 'submissiondate', 'Id10011', 'Id10023').filter(causes__isnull=True)[:NUM_TABLE_ROWS].prefetch_related("causes", "coding_issues", "location")
            
            # List the VAs with Indeterminate COD
            vas_with_indeterminate_cod = user_vas.only('id', 'location_id', 'Id10007', 'Id10010', 'Id10017',
                'Id10018', 'submissiondate', 'Id10011', 'Id10023').filter(causes__cause="Indeterminate")[:NUM_TABLE_ROWS].prefetch_related("causes", "coding_issues", "location")

            context['issue_list'] = self.get_context_for_va_table(vas_to_address)
            context['indeterminate_cod_list'] = self.get_context_for_va_table(vas_with_indeterminate_cod)

            # If there are more than NUM_TABLE_ROWS show a link to where the rest can be seen
            context['additional_issues'] = max(context['vas_uncoded_overall'] - NUM_TABLE_ROWS, 0)
            context['additional_indeterminate_cods'] = max(user_vas.only('id').filter(causes__cause="Indeterminate").count() - NUM_TABLE_ROWS, 0)
